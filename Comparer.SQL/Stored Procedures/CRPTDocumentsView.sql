	...
	if @CRPTNumber is not null begin
		...
		if (@DocNumber is not null) begin
			...
			where
				...
				(d.DocNumber like '%' + @DocNumber + '%')
			order by
				d.CRPTCreatedDate desc
			...
			where
				...
				(@ReceiverId	is null or (@ReceiverId		is not null and d.ReceiverId	= @ReceiverId))				and
				(@StatusId		is null or (@StatusId		is not null and d.StatusId		= @StatusId))				and
				(@DocDateFrom	is null or (@DocDateFrom	is not null and cast(d.CRPTCreatedDate as date) >= @DocDateFrom))		and
				(@DocDateTo		is null or (@DocDateTo		is not null and cast(d.CRPTCreatedDate as date) <= @DocDateTo))			and
				(@Id			is null or (@Id				is not null and d.Id			= @Id))						and
				(@DocNumber		is null or (@DocNumber		is not null and d.DocNumber like '%' + @DocNumber + '%'))
			order by
				d.CRPTCreatedDate desc
