create proc DeliveryServiceOrderCompensationSave_v2
(
	...
	@CompensationSum		float,					-- Сумма компенсации
	@CompensationFail		bit,					-- Признак отказа от компенсации
	...
	begin try
		...
		begin
			...
			set
				...
				a.CompensationSum			= @CompensationSum,
				a.CompensationFail			= @CompensationFail
			from 
		...
		begin
			...
			insert ApiDeliveryServiceOrdersInfo 
				(InnerId, LabelDowloadCount, CreatedDate, ChangedDate, CompensationNumber, CompensationDate, CompensationSum, CompensationFail)
			values 
				(@InnerId, 0, getdate(), getdate(), @CompensationNumber, @CompensationDate, @CompensationSum, @CompensationFail)
