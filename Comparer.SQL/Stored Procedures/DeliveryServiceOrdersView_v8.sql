create proc DeliveryServiceOrdersView_v8
(
...
begin
	...
	if @GroupId is not null begin
		...
		insert into #tempIds
			...
			where
				...
				(@ProblemInTK is null		or (@ProblemInTK is not null		and (op.ProblemInTK = @ProblemInTK or @ProblemInTK = 0 and op.ProblemInTK is null))) and
				(@Compensation is null		or (@Compensation is not null		and 
					(
						(@Compensation = 3 and oi.CompensationFail = 1) 
						or 
						(@Compensation = 2 and oi.CompensationSum is null)
						or 
						(@Compensation = 1 and oi.CompensationSum > 0)
					)
				))
	...
	select 
		...
		isnull(oi.CompensationFail, 0) as 'CompensationFail',
		...
		#tempIds t 
			...
			left join (
				...
				from
					...
					JOIN #tempIds t ON oc.OrderId = t.InnerId
