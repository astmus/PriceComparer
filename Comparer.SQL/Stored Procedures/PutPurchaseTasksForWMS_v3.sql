create proc PutPurchaseTasksForWMS_v3 as
begin
	...
	begin try
		...
		insert WmsExpectedReceiptDocuments 
		(
			WarehouseId, PublicId, Number, WmsCreateDate, ExpectedDate, DocStatus, 
			DocSource, IsDeleted, Priority, Comments, AuthorId, Operation, CreateDate
		)
		select 
			1, t.PublicGuid, ('crm-' + cast(t.CODE as varchar(10))), getdate(), isnull(t.DELIVERYDATE, getdate()), 1,
			1, 0, dbo.PriorityForExpectedReceipt(t.Code), '', '695B7A37-3A07-4CAE-A288-7E1C1D20A23C', 1, t.PURCHASEDATE
		from 
			#purchaseTaskIdsForExport e
			join PurchasesTasks t on t.PublicGuid = e.TaskId 
		where 
			not exists (select 1 from WmsExpectedReceiptDocuments rd where rd.PublicId = e.TaskId and rd.WarehouseId = 1)
		...
		from 
			#purchaseTaskIdsForExport e 
			join PurchasesTasks t on t.PublicGuid = e.TaskId
			join WmsExpectedReceiptDocuments d on d.PublicId = t.PublicGuid and d.WarehouseId = 1
			join PurchasesTasksContent c on c.CODE = t.CODE and c.ARCHIVE = 0
			join PRODUCTS p on p.SKU = c.SKU
		where 
			not exists (select 1 from WmsExpectedReceiptDocumentsItems i where i.DocId = d.Id and i.TaskId = t.PublicGuid and i.Sku = c.SKU)

		-- 3. Добавляем Объединенные задания на закупку
		-- Добавляем Объединенные задания на закупку в таблицу "Ожидаемые приемки"
		insert WmsExpectedReceiptDocuments 
		(
			WarehouseId, PublicId, Number, WmsCreateDate, ExpectedDate, DocStatus, 
			DocSource, IsDeleted, Priority, Comments, AuthorId, Operation, CreateDate
		...
		select 
			1, t.PublicGuid, ('crmj-' + cast(t.Id as varchar(10))), getdate(), getdate(), 1,
			1, 0, 3, 'Объединенное задание на закупку', '695B7A37-3A07-4CAE-A288-7E1C1D20A23C', 1, t.CreatedDate
		from 
			#purchaseTaskIdsForExport e
			join PurchasesTasksJoined t on t.PublicGuid = e.TaskId
		where 
			not exists (select 1 from WmsExpectedReceiptDocuments rd where rd.PublicId = e.TaskId and rd.WarehouseId = 1)

		-- Добавляем товарные позиции Объединенного задания на закупку в таблицу "Товары ожидаемой приемки"
		insert WmsExpectedReceiptDocumentsItems (DocId, TaskId, ProductId, Sku, Quantity, ForOrder, CreateDate)
		select d.Id, t.PublicGuid, p.ID, p.SKU, c.PLANQUANTITY, 
				case when c.ItemType = 2 then cast(0 as bit) else cast(1 as bit) end,
				getdate()
		from 
			#purchaseTaskIdsForExport e 
			join PurchasesTasksJoined t on t.PublicGuid = e.TaskId
			join WmsExpectedReceiptDocuments d on d.PublicId = t.PublicGuid
			join PurchasesTasksJoinedLinks l on l.JoinTaskId = t.Id
			join PurchasesTasksContent c on c.CODE = l.TaskId and c.ARCHIVE = 0
			join PRODUCTS p on p.SKU = c.SKU
		where 
			not exists (select 1 from WmsExpectedReceiptDocumentsItems i where i.DocId = d.Id and i.TaskId = t.PublicGuid and i.Sku = c.SKU)
