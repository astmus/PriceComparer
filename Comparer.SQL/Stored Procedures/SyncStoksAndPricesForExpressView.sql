create proc SyncStoksAndPricesForExpressView
(		
...
begin
	...
	declare
		@WarehouseId	bigint		--- Идентификатор склада в системе Маркетплейса (Express склад)
	select
		@WarehouseId = 
			case 
				when @SiteId = '5D4463C6-EBA6-4DD5-8618-D07AE3A23D9A'	then 1020000314928000	-- OZON
				when @SiteId = 'B30E878A-C805-436E-9EAC-7865337DBEA8'	then 145431				-- Yandex 
			else null
			end	
		...
		select 
			...
			case
				when sp.SP_WITHDISCOUNT < 3000	then 0
				else iif( (p.ManId in ('DD6B6618-7C30-494F-963C-140C642999FE')), -- Бренды, по которым выдаем фактический остаток
					isnull(ps.FreeStock, 0), isnull(inst.Instock, isnull(ps.FreeStock, 0)))	
			end							as 'Stock',
