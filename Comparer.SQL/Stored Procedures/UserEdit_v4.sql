...
begin
	...
	select @oldDepartmentId = DepartmentId from Users
	where ID = @UserId
	...
	begin try 
		...
		begin 
			...
			if(@DepartmentId <> @oldDepartmentId) or (@Status in (2,3))
			begin
				delete from UsersBounds 
				where UserId = @UserId or ParentId = @UserId
			end
