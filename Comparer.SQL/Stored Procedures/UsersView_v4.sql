...
begin
	...
	if @Email = '' set @Email = null
	    ...
	    where
			...
			(@RoleId is null or (@RoleId is not null and r.Id = @RoleId)) and
			(@FIO is null or (@FIO is not null and concat(LTRIM(A.LastName), ' ', LTRIM(A.FirstName),' ', LTRIM(A.Patronymic)) like ('%' + @FIO + '%')))
		order by 
			A.NAME
end
