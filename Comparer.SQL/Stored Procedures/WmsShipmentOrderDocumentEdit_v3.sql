create proc WmsShipmentOrderDocumentEdit_v3 
(
	...
	begin try 
		...
		if @Operation = 1 begin
			...
			if @IsDeleted = 1 begin
				...
				exec @result = WmsShipmentOrderDocumentEdit_v3 3, @PublicId, @ClientId, @Number, @ShipmentDirection, @OrderSource,
			...
			if exists (select 1 from WmsShipmentOrderDocuments where PublicId = @PublicId) begin
				...
				exec @result = WmsShipmentOrderDocumentEdit_v3 2, @PublicId, @ClientId, @Number, @ShipmentDirection, @OrderSource,
					@DocStatus, @WmsCreateDate, @ShipmentDate, 
					@DeliveryAddress, @RouteId, @IsShipped, @IsPartial, @IsDeleted, @Comments, @AuthorId, @err out
				-- Если корректировка закончилась неудачей, делаем rollback транзакции
				if @result = 1 begin
			...
			(
				WarehouseId, PublicId, ClientId, Number, WmsCreateDate, ShipmentDate, ShipmentDirection, OrderSource, 
				DeliveryAddress, RouteId, DocStatus, IsShipped, IsPartial, IsDeleted, AuthorId, Comments, Operation
			)	
			values
			(
				1, @PublicId, @ClientId, @Number, @WmsCreateDate, @ShipmentDate, @ShipmentDirection, @OrderSource,
				@DeliveryAddress, @RouteId, @DocStatus, @IsShipped, @IsPartial, @IsDeleted, @AuthorId, @Comments, @Operation)
		...
		if @Operation = 2 begin
			...
			(
				Id, WarehouseId, PublicId, ClientId, Number, WmsCreateDate, ShipmentDate, ShipmentDirection, OrderSource, 
			...
			from WmsShipmentOrderDocumentsItems 
			where DocId = @oldDocId
			-- Удаляем Товарные позиции по устаревшему Документу
			delete WmsShipmentOrderDocumentsItems 
			where DocId = @oldDocId
